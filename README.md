...
| Profile switching | on keyDown, `switchProfile` over the plugin socket, confirmed by the pressed key's `willDisappear`; stock OpenDeck ignores that event from plugins, so after 400 ms without it the plugin uses `opendeck --process-message '{"event":"switchProfile",…}'` for the rest of its lifetime |
