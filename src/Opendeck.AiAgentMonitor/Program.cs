...
if (args.Length > 0 && args[0].StartsWith("--"))
{
    ...
    switch (args[0])
    {
        ...
        case "--render":
        {
            ...
            var q = new ProviderQuota { Provider = Provider.Claude, FetchedAt = now, Plan = "max", Windows = new[] { new QuotaWindow("5h", 36, now.AddHours(2.3)), new QuotaWindow("7d", 67, now.AddDays(1)), new QuotaWindow("7d", 41, now.AddDays(1), "Fable") } };
            Save("sample-quota", r.QuotaKey(Provider.Claude, q, now));
            Save("sample-quota-fable-warn", r.QuotaKey(Provider.Claude, q with { Windows = new[] { q.Windows[0], q.Windows[1], q.Windows[2] with { UsedPct = 86 } } }, now));
            Save("sample-quota-fable-full", r.QuotaKey(Provider.Claude, q with { Windows = new[] { q.Windows[0] with { UsedPct = 100 }, q.Windows[1], q.Windows[2] with { UsedPct = 100 } } }, now));
            Save("sample-quota-no-fable", r.QuotaKey(Provider.Claude, q with { Windows = new[] { q.Windows[0], q.Windows[1] } }, now));
